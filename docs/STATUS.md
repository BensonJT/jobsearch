...
**Data center employers in the corpus:** QTS (workday, 354 rows / 305 active), Digital Realty (oracle_orc, 206 / 173), now NTT (224). Not registered: Stream Data Centers (applied 3/31), Equinix, CyrusOne, Vantage, Aligned, CloudHQ, Iron Mountain, Compass, STACK, Sabey, EdgeCore, DataBank. Candidates for registration if the user wants the Ashburn corridor covered.

**Overnight FULL launched at 10:55 by hand** (user away for a few hours): `printf '1\ny\n' > logs/launch_answers_20260926_1055.txt; setsid nohup bash launch.sh < that > logs/launch_manual_20260926_1055.out`. The launcher's own log is `logs/launch_<stamp>.log`. Preset `sweep:--llm-top 100|top|maint`, ~3.75 h. First attempt failed because stdin was redirected from /dev/null after the pipe (menu read nothing); the second attempt is the live one. When it finishes: read the new `Top_Jobs_*.md` in the vault, check whether NTT JR102006 was judged, and look at `vw_board_health` for the new board.

**NEW adapter candidate, 2026-09-26: ADP Workforce Now.** Small government contractors use it (Agile5 Technologies surfaced via Glassdoor). Public JSON, no auth, verified live: list `https://workforcenow.adp.com/mascsr/default/careercenter/public/events/staffing/v1/job-requisitions?cid=<cid>&ccId=19000101_000001&lang=en_US&$top=50` returns `jobRequisitions[]` with `itemID`, `requisitionTitle`, `postDate`, `requisitionLocations[]`, `workLevelCode`; detail `.../job-requisitions/<jobId>?cid=...` returns the same plus `requisitionDescription` (HTML), `clientRequisitionID`, `customFieldGroup`. Registry identifiers would be `cid` (GUID) and `ccId`. Same shape as the Workday adapter (list endpoint carries no JD; per-posting detail call). Not built; add as `adp` platform when a second ADP employer shows up or Jeff asks.

**Still open (unchanged from 9/24):** `judge2 eval --compare 7533bcce2a17 36ca03157cf2`; the 1 unparseable gold row; per-line miss dump; `finder.py retrain` (67+ new human labels, RETRAIN REMINDER fired 9/24); nine gold unmet lines `never_listed`; `judge2 status` ignores background flags; `gold_ingest._merge` blind/seen flaw; coverage calibration warning; weekly retrain step. NEW: SPRINT_PLAN §33 (Jev typed-decision tier) is PROPOSED, research first; the user is doing Perplexity research (prompts in the vault article backlog item 7).


## HANDOFF 2026-09-24 ~12:30: gold score run DONE under a NEW prompt_version (fact sheet edited), NOT PASSED; NFCU pair graded bullseye; retrain due tonight; nothing running

**What changed before the run.** The judge fact sheet (`judge2_background.local.md`, copy of the vault's `Tools/judge2_background.md`) gained the facts from resume bullets 321-367 (Smartsheet initiative tracker + Slack lookup, OCC request-intake Slack workflows, benefits-realization governance, FP&A close automation, 2020-2022 portfolio results; Smartsheet and Slack workflows in Tools). Pending the user's read-through. `prompt_version` moved `7533bcce2a17` -> `36ca03157cf2`, so every gold row was re-judged. The vault's bullet-authoring skill now has a §10b that says to do this after every bullet batch (coverage picks bullets up automatically from `vw_resume_bullets`; the judge does not).

**The run.** "Gold score only" preset (`judge2run|judge2eval|maint`), 08:26-12:07, 3 h 40 m, exit 0. 112 gold rows (was 90 on 9/21: the 9/23 blind sheet import added 22), 111 reviewed, **1 unparseable** (which posting/model not yet looked at). Eval: **catch 0.56 (18 in the denominator; bar 0.70), strict 0.28, all-fails 0.74, agree 0.77 (31; bar 0.85), 43/86 gold unmet lines rated unmet. NOT PASSED.** 9/21 reference on 90 rows: catch 0.77, agree 0.77, 31/67 lines. Two variables moved at once (rows and background), so nothing can be read off this run alone. Full notes and the experiment in SPRINT_PLAN §32.

**NEXT, in order.** (1) `finder.py judge2 eval --compare 7533bcce2a17 36ca03157cf2` (with the background flags) -- the same-rows comparison already exists as `--compare`; it isolates the fact-sheet effect on the ~90 common rows. Then the 22 new rows alone. Eval-only, minutes, no API call. (2) Find the unparseable row. (3) Per-line miss dump by requirement kind for the 43 misses. (4) `finder.py retrain` tonight: RETRAIN REMINDER fired 9/24 00:57, 67 new human labels. (5) Last night's overnight FULL (00:17-00:59, 42 min) was correct and short: only 7 top-100 rows were new since 9/23, the rest served from cache; it was FULL, not FULL + GOLD SCORE, which is why no gold score ran overnight.

**Grades recorded today.** NFCU AI Business Transformation Analyst `8de73fde12a47f4cbf06` and Senior Program Manager (Learning & Talent Development) `272a8f13316dcb3f5e47`: `mark build --grade bullseye --basis seen`. The second title matched two postings (sibling "Program Manager (Training and Talent Strategy)"), so `mark` by title refused; marked by id. 12 packages built by another session this morning were audited in the vault (no hard-gate or out-of-record claims); their grades come when the user reviews them.

**Vault-side facts that matter here.** Two ATS rejections landed 9/24 (Elevance PIM Sr, CVS S&O Mgr), both IC-classified reqs; the resume DB's Verizon block titles now carry "(individual contributor)" and the duty statements open with "no direct reports" by default, so coverage evidence text changed slightly (re-synced on the next sweep). NFCU screening record: 4 of 9 passed initial screening, all IC titles; all 5 Manager I/II rejected.

## HANDOFF 2026-09-22 01:40: §31 bridge track MERGED and RUN LIVE (schema v22); full refresh DONE after a WSL crash mid-run; judge round 8 NOT PASSED but the J2 column is filled; nothing running, no worktrees

**State.** main = `3e93c3c` (merge of `bridge-track` at `b891266`, plus two crash fixes). Live DB v22, backed up pre-migration outside the repo. No worktrees, no branches but main, nothing running. Full suite at the bridge tip before the merge: 587 passed + the 1 known local-file failure.

**The run.** Retrain promoted all five lens models (process .931, technical .909, ai .943, required .824, bullseye .807); `required_embed` failed its own gate again (stack AUC 0.706 vs 0.71) and was not promoted, same near miss as 9/19. Fit sweep: 141 boards, 5,080 JDs fetched, postings 92,947 -> 100,482 (85,893 active). Full rescreen of all 85,893 rows: candidate 239 / review 975 / reject 84,679 -- the newly promoted models moved +6 reject / -1 review, i.e. the corpus was already scored under equivalent models. required-embed scored 746 of a population of 791 (558 OOF, 188 fresh, 45 skipped for no Required line surviving the top-18 cut). Second judge: 100 postings by rank, `7533bcce2a17`, 0 unparseable, 3 h. Reports written for both dates.

**A WSL crash killed the first attempt at ~20:39, in the required-embed scoring stage. Root cause found and fixed, not worked around:** `store.connect()` left DuckDB's `memory_limit` at its default 80% of RAM = 6.2 GiB of an 8 GB WSL VM, so a stage holding a sentence-transformer encoder in the SAME process overcommitted the VM. Now capped at 3500MB with `JOBSEARCH_DUCKDB_MEMORY_LIMIT` to override; DuckDB spills to `temp_directory` at the limit, so the cost is speed on a big rescreen, never correctness. Nothing was lost: postings commit board-by-board, the DB replayed its WAL clean, and the required-embed cache `_save()`s after EVERY batch (22,725 texts survived, so the resumed run had to embed only 9). **Second defect found while waiting:** `pipeline.daily()` passed both `full` and `since` to `screen()`, which `_candidate_sql` ANDs, so `sweep_ats.py --full-screen` screened only the rows that sweep had just touched -- 7,625 of 85,893 on the first attempt. Now `since=None if full else since`, the pairing `coverage_stage` already used. Both fixes are `3e93c3c` with tests.

**Round 8 (§30 role shape), NOT PASSED.** catch 10/13 = 77% (bar 70, cleared; strict fails-only 31%), agree 17/22 = 77% (bar 85, missed), 31/67 gold unmet lines rated unmet, 18 of 89 common postings changed verdict against round 5. So `judge2_bar_passed` is false and J2 stays a REPORTED column that never moves the rank -- confirmed live: every top row's `effective_required_source` reads `judge` or `human`, never `judge2`. Full miss list in `db/judge2_eval8_followup_20260921.log`. The three catch misses all came back `meets`; two agree misses flipped to `fails` since round 5 and three are capped at `partial` (suspect the new soft-adjacent 0.5 rule or the discarded-line cap -- both are constants, so `judge2 rederive` can test changes with no API call). Six gold unmet lines rated `adjacent` print as misses and need reading separately; three clearance/domain lines are still `never_listed`.

**First live bridge sweep (§31): 8 boards, 0 failed, 70 postings, 18 seconds, ring 1 only.** Walmart 29, CarMax 19, Weis 15, AutoNation 7. Zero from Kohl's, Wegmans, Target, Starbucks -- and each zero correctly skipped its close pass, so nothing was retired on an empty pull. **Kohl's needs `allow_no_state` on its place row** (its location text is city-plus-store-number only, e.g. "Springfield (0350)"); that column is per-place and applies to every employer, so it is the user's call. Target hit the 5-page cap on `Remote` and correctly refused to close on that place. Every row came back voice `low`.

**Boards worth watching.** T-Mobile is clamped (true total 2,105): the stored partition plan on `timeType` pulled 2,099 and flagged itself truncated, so it never close-passes -- working as designed. AT&T 1,251 and Verizon 1,218 are under the ceiling. USAJobs ran live for the FIRST time (credentials are in `.env` now): 3,208 rows, 300 new, always truncated by design.

**NEXT.** (1) Read the round 8 misses with the user; tune constants (free, `rederive`), then the sheet, then the prompt. (2) The `allow_no_state` ruling for Kohl's, and a second bridge sweep once the places file gains it. (3) Bridge adapter candidates still wanted, each behind a probe: Jibe-fronted iCIMS, iSolved Hire / ApplicantPro, Oracle ORC multi-brand (Harris Teeter is registered nowhere yet -- it has no places-scoped adapter). (4) Still open from before: nine gold unmet lines `never_listed` (three clearance); `judge2 status` ignores background flags; `gold_ingest._merge` blind/seen flaw; coverage logs "no calibration matches the running config"; weekly retrain as a scheduled step; `required_embed`'s gate has now missed twice by ~0.004, so either the gate or the stack needs a decision.

## HANDOFF 2026-09-21 ~17:00: round 8 IN FLIGHT (detached); §31 bridge track BUILT + AUDITED + FIXED on branch `bridge-track`, fix commit NOT re-audited, NOT merged; read this first

**1. Live round 8 (§30 judge) is running detached.** Started 14:16 after the user said he had read the background sheet and said go. `finder.py judge2 run --eval-set --background file --background-file judge2_background.local.md`, minimal thinking, prompt_version `7533bcce2a17`, 90 postings. Still running at 16:59 (2 h 42 min; the run holds the DuckDB lock for its whole life, NOTHING else can open the live DB, read-only included). Its stdout log (`db/judge2_live_eval8_20260921.log`) is block-buffered and stays nearly empty: that is NOT a hang; check liveness with `pgrep -af "judge2 run"` and the mtime of `db/jobsearch.duckdb.wal`. A detached follow-up (`db/judge2_r8_followup.sh`, untracked) waits for the run to exit, then writes `judge2 status`, `judge2 eval` and `judge2 eval --compare 0d6b4871f44a 7533bcce2a17` to `db/judge2_eval8_followup_20260921.log`, ending `=== ALL DONE`. **When it is done:** read that log (bar: catch >= 70%, agree >= 85%; round 5 reference 62% / 85%); in the line report a gold unmet line the judge rated `adjacent` prints as a miss: list those separately; read misses with the user; tune constants (free, `judge2 rederive`), then sheet, then prompt; then `finder.py retrain` (six lane grades changed 9/21); delete the two untracked `db/judge2_r8*.sh` scripts; remove worktree `~/jobsearch_wt_j2shape` + branch `judge2-shape` (merged). If the follow-up log never appears, run the three commands by hand with the background flags. Next time launch with `PYTHONUNBUFFERED=1`.
**2. §31 bridge-role track (spec in SPRINT_PLAN §31).** Worktree `~/jobsearch_wt_bridge`, branch `bridge-track`, cut from main `1cd11d8`: `2644bed` (Sonnet build, schema v22, 26 tests) + `bd8533c` (nine audit fixes; worker reports 46 bridge tests, full suite 586 passed + the 1 known local-file failure). Two Sonnet auditors plus an orchestrator read found, all now addressed in `bd8533c`: a bridge row on a platform without places support fell through to a WHOLE-BOARD pull (now refused at three layers); places-file comment lines and a BOM; location forms (one-dash "ST - City", full state names, opt-in city-only store text); remote synonyms; a zero-row place closes nothing; staleness window (10 days) instead of forever-open rows; `pseudo_negatives` + two views + `PostingIndex` filtered to `track='fit'`; NEW mark keyed on sweep run_id; voice flags on word boundaries; `--limit` after `--track`. Verified sound before the fixes: the close pass (ring isolation, tag carry-forward, two-place rows, reopen, fit/bridge isolation both ways), posting ids byte-identical for the fit track, board health per track, v22 migration backfill. **NEXT for §31:** (a) orchestrator re-audit of `bd8533c` alone (read the diff; points to check: the rewritten zero-row test, NEW comparison changed `>=` to `>`, the `\bsql\b` short-circuit in the voice flag, the three-layer places guard); (b) full suite; (c) AFTER round 8 releases the DB: back up the live DB outside the repo (pre-v22), merge (the auto-mode classifier denied the orchestrator's `git merge` into main for §30; the user ran `! cd ~/jobsearch && git merge --no-ff <branch>` himself), connect once to migrate to v22; (d) the user's private registry gains `track` / `evergreen` columns and the staged bridge rows (kept in his vault, never here), `bridge_places.csv` and `bridge_employer_order.csv` already exist there; (e) first scoped sweep: `sweep_ats.py --track bridge` (ring 1), then `finder.py bridge`; NEVER run a bridge row without the places file; (f) adapters still wanted, each behind a probe: a Jibe-fronted iCIMS career site (public JSON search found), an iSolved Hire / ApplicantPro board (public JSON list found), an Oracle ORC multi-brand board that needs a brand or location filter; sites behind a bot challenge stay check-by-hand, never worked around.
**3. Registry.** Two telecom employers were added to the user's private fit-track registry today (both Workday, probe-verified; one reports the 2,000 clamp, so check its `vw_board_health` row after the first sweep). First sweep of them also waits for the DB.
**Still open (unchanged).** Nine gold unmet lines `never_listed` (three clearance lines); `judge2 status` ignores background flags; `gold_ingest._merge` blind/seen flaw; coverage calibration warning; weekly retrain as a scheduled step.

## HANDOFF 2026-09-21 evening: §30 AUDITED, FIXED, MERGED (schema v21); live DB migrated and rederived; round 8 waits only for the user's go

**State.** main = merge of `judge2-shape` (`2c25986`). Orchestrator audit (three Sonnet readers plus a line-by-line read) found two defects and one rule needing a user ruling, all fixed on the branch before the merge: (1) the shape-only path (zero surviving required lines, shape `fits`) returned `meets` without checking `lines_discarded`: now capped at `partial`; (2) the bridge applied to ANY adjacent hard gate, so a title tool rated `adjacent` came back `meets`: now only a `years_function` gate bridges, any other adjacent hard gate is `partial`; (3) user ruling: when more than `SOFT_ADJACENT_MEETS_MAX_FRACTION` (0.5) of the soft required lines are `adjacent` the call is `partial` at best. Verified clean: 4x4 combination table, shape boundaries, 500-input backward compatibility, evidence guard on `adjacent`, prompt rules, payload unchanged, v21 migration idempotent, views, diagnostics, fixtures hold no personal facts. Full suite at the branch tip: 540 passed + the 1 known local-file failure. Kept as built (cosmetic): gate decisions use the old why strings, not a `gate:` prefix.
**Live DB.** Backed up outside the repo (pre-v21), migrated to v21, `judge2 rederive` recomputed 264 reviews. Round 5 (`0d6b4871f44a`) re-scores exactly as before: catch 8/13 = 62%, agree 17/20 = 85%. Dry run for round 8: 90 postings, ~705K estimated input tokens, `prompt_version 7533bcce2a17`, models unchanged, same seven payload fields; the working background copy is in sync with the user's sheet.
**NEXT, in order.** (1) The user says he has read the background sheet and says go. (2) `JUDGE2_LIVE_OK=1 finder.py judge2 run --eval-set --background file --background-file judge2_background.local.md`, detached with a log, minimal thinking, VPN off (expect 2-2.5 h). (3) `judge2 eval` with the background flags, then `--compare 0d6b4871f44a 7533bcce2a17`. (4) Read misses with the user; tune constants (free, `rederive`), then sheet, then prompt. In the line report a gold unmet line the judge rated `adjacent` shows as a miss: read those separately. (5) `finder.py retrain` (six lane grades changed). (6) Remove worktree `~/jobsearch_wt_j2shape` and branch `judge2-shape`.
**Still open.** Nine gold unmet lines `never_listed` (three clearance lines); `judge2 status` ignores background flags; `gold_ingest._merge` blind/seen flaw; coverage calibration warning; weekly retrain as a scheduled step.

## HANDOFF 2026-09-21 afternoon: gold labels re-ruled, §30 (role shape) SPECIFIED and BEING BUILT on branch `judge2-shape`; read this first

**What happened after round 7.** The user re-graded 17 disputed gold rows line by line (sheet in the vault tools folder, `Judge2_Rulings_Sheet_20260921.csv`): 8 required-fit labels changed (2 to meets, 6 to fails), 6 lane grades changed. Gold sources were edited to match and re-ingested; gold is now 68 fails / 22 meets. Re-scored with no API call: round 5 (`0d6b4871f44a`, minimal thinking) catch 8/13 = 62% / agree 17/20 = 85%; round 7 (`2ddfcc1a47a1`, thinking high) catch 54% / agree 90%. Agree is at the bar; every remaining catch miss falls under a §30 rule (domain qualifier over an or-list, systems named in a line, "directly related" read against responsibilities, role shape). The background sheet was extended from the rulings and the working copy `judge2_background.local.md` refreshed, so the NEXT run has a new prompt_version regardless.
**Build finished 2026-09-21 ~15:00, NOT yet audited or merged:** branch `judge2-shape` commits `832e076` (code + tests) and `a8a94f8` (spec note); the worker reports 526 passed + 1 known local-file failure. Its own listed deviations to check in the audit: it changed what `lines_discarded` counts instead of adding a column; it kept the old `derive_why` wording for gate decisions; it left the J2 cell unchanged (Why text only); an unclear hard gate blocks the bridge. **Original in-flight note:** A Sonnet worker is building §30 in worktree `~/jobsearch_wt_j2shape` (branch `judge2-shape`, cut from `4355b4b`): `responsibility` section, `adjacent` verdict, eight reading rules in the prompt, `lines_fit` + `shape_fit` derived in code, schema v21, unparseable-response diagnostics (`finishReason` + token counts). It may not touch this checkout, the live DB or the network. If the session that launched it is gone, the branch and its commits are still there: check `git -C ~/jobsearch_wt_j2shape log --oneline` and whether its §30 heading says BUILT-NOT-RUN.
**NEXT, in order.** (1) Orchestrator audit of `judge2-shape` line by line (every earlier worker branch had a real defect): derivation table vs §30.3, the evidence guard on `adjacent`, the v21 migration, that a discarded responsibility line does not cap the call, no personal facts in fixtures. Full suite. (2) Back up the live DB outside the repo, merge, migrate to v21, `judge2 rederive`. (3) The user reads the updated background sheet (it is what leaves the machine) and says go. (4) Dry run, then live round 8 at minimal thinking (expect 2-2.5 h: output roughly doubles), `judge2 eval`, `--compare` against `0d6b4871f44a`. (5) Read misses with the user; tune constants (free), then sheet, then prompt. (6) `finder.py retrain` so the six changed lane grades take effect. (7) `.personal_patterns` scan, push (local: `4355b4b` + this note).
**Still open.** Nine gold unmet lines are `never_listed` by the judge (three clearance lines): extraction gap. `judge2 status` ignores background flags. `gold_ingest._merge` blind/seen flaw. Coverage calibration warning. Weekly retrain as a scheduled step.

## RESULT 2026-09-21 01:10: round 7 (thinking=high) DONE; thinking did NOT help; nothing running

**Numbers (prompt_version `2ddfcc1a47a1`, same prompt and background sheet as rounds 5-6, only `JUDGE2_THINKING=high`, 6 h 5 min for 90 postings).** 86 reviewed, 4 unparseable. catch 6/13 = 46% (round 5: 62%; bar 70), agree 18/23 = 78% (round 5: 78%; bar 85), catch over all judged human-fails 43/61 = 70%, 36 of 64 gold unmet lines rated unmet (round 5: 39). NOT PASSED. `judge2 eval --compare 0d6b4871f44a 2ddfcc1a47a1`: 13 of 85 common postings changed verdict; eight moved more lenient (fails -> partial/meets or partial -> meets), including two catch-set rows that round 5 had caught as `partial` and round 7 calls `meets`; three moved stricter (one of them an agree-set row, now a new agree miss); two differ only because one side had no verdict. With thinking on, the model argues itself into `met` on or-list years lines more often, not less.
**The 4 unparseable rows are output-cap truncation, confirmed.** A diagnostic re-ask of one of them returned `finishReason=MAX_TOKENS`, `thoughtsTokenCount=16381` of the 16,384 cap, and an EMPTY answer part: the model spent the whole budget reasoning. Each such row cost two ~6-8 min calls. That single call totalled 21.4K tokens with no 429, so the free-tier per-minute token limit did not bind on it; no 429 or retry appears anywhere in the round 7 log.
**Conclusion.** Minimal thinking was not the handicap. The remaining misses are the same family as before (or-list years lines read generously against a generic listed item; held clearances the line extractor never lists), so the next move is still the user's rulings and the `never_listed` extraction gap, not the model setting. Round 5's configuration (minimal thinking, ~1.75 h per 90) stays the reference.
**Open code items.** (1) `judge2.run` discards an unparseable response without logging `finishReason` / `usageMetadata`: log them. (2) If thinking is ever used again, cap it (`thinkingLevel` below high, or a larger `JUDGE2_MAX_OUTPUT_TOKENS`) and raise `THINKING_EXPECTED_OUTPUT_TOKENS` (real use was 10-16K, budgeted 6K). (3) 9 gold unmet lines are `never_listed` by the judge (three are clearance lines): an extraction gap in `required_lines`, independent of the model. (4) Carried over: the zero-required-lines gap, `judge2 status` ignoring background flags, `gold_ingest._merge` blind/seen flaw, coverage calibration warning, weekly retrain step.
**Still waiting on the user.** Rulings on the or-list years lines inside a foreign domain, the three gold labels that conflict with the user's own rulings, and the two facts the background sheet may be missing (technology-transformation years, project-management years).
