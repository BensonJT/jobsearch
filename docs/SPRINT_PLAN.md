...
**Why.** Two problems, one shape. (1) Judgment: the Gemma second judge (§25, §29, §30) has not passed the bar in eight rounds (9/24: catch 0.56 against 0.70, agree 0.77 against 0.85) and has a hard time matching the gold sheet. (2) Cost while unemployed: every posting sent to an LLM judge is paid tokens and free-tier minutes, so the judge runs on top-N only and the rest of the corpus is ranked by the deterministic layers alone. Jev is positioned as a "System One" decision model: it takes application state plus declared questions and returns typed choices with probabilities and confidence, no prose, at a reported fraction of LLM cost and latency. That is the shape of the lens grades and the required-fit call, not the shape of a cover letter.

**The proposed tier, in pipeline order.**

1. Free deterministic screen exactly as today: rules, the five TF-IDF lens models, `required_embed`, coverage. No change.
2. **Gate:** only postings that score well on the free layers go to Jev. The threshold is a constant (`JEV_GATE_MIN_FINAL_SCORE` or a band), so the spend is capped and tunable with no code change. Cache by `(posting_id, description_hash, jev_prompt_version)` exactly as `judge2_reviews` is keyed, so a posting is never re-sent for the same JD and question set.
3. **One Jev call per posting, several questions in the call.** State = the JD text plus the user's background lenses (the same evidence the judge already gets: `judge2_background.local.md` or a compact per-lens fact sheet). Declared questions, each typed: process-lens fit (bullseye / adjacent / stretch / wrong), technical-lens fit, applied-AI-lens fit, level fit (too_low / in_range / stretch_up / too_high), required-fit (meets / partial / fails), and a small set of yes/no gates the reading rules already name (clearance required, people-management required, on-site cadence). Jev returns a typed value plus a probability for each, in one response, which is the relative ranking across questions the user is asking for.
4. **Store as a new column family, never overwrite.** `jev_reviews` beside `judge2_reviews`; `vw_lens_fit` gains a reported J3 column the way J2 was added. Rank does not move until an eval bar passes (the same discipline as `judge2_bar_passed`).
5. **Gemma last, if at all, and with a different question.** If Jev carries the categorical calls, the remaining job for an LLM is the one Jev cannot do: read a required line verbatim and cite evidence. Try the judge only on postings Jev rates high, and consider a narrower prompt (one line, one verdict, one citation) rather than the whole Required block. If Jev's required-fit probability already tracks gold within the bar, Gemma may have no job left.

**Evaluation before trust (the MSA rule).** Jev is a new appraiser and gets the same treatment the judge got, using the machinery that already exists: run it on the gold set (112 rows), `judge2 eval`-style catch / agree / all-fails against `vw_report_feedback_blind`, a `--compare` against the best Gemma prompt_version on the same rows. Add two checks the typed output makes possible for the first time: repeatability (the same posting sent twice, does the probability move) and calibration (bin the returned probabilities and check that 0.8 means right about 8 times in 10). If the calibration holds, the probability becomes a rank input instead of a categorical flag, which no prior judge could offer.
