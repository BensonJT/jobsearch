...
- **Final score** (`pipeline.combine`): `SCORE_COMPONENT_WEIGHTS` = content 0.50 · level 0.20 · location 0.15 · pay 0.10 · title 0.05. `screens.rule_score` holds the profile score = the four non-content components weighted on 0–100 (`rules.profile_score`); final = 0.5·content + 0.5·profile (a low-data `fit_weight` replaces the content weight). Content = 100·fit, averaged with the calibrated embedding score when present. No content → profile score capped at `NO_CONTENT_CAP` (60). Then `TIER_CAP`, minus `FLAG_PENALTY` (5) per flag up to `FLAG_PENALTY_CAP` (25), then the LLM blend. Bands unchanged.
