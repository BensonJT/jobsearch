...
## IN FLIGHT 2026-09-20 evening: round 7 (thinking ON) is running DETACHED; read this first

**What is running.** `db/judge2_r7.sh` (untracked helper, delete when done), started 19:05 EDT: `JUDGE2_THINKING=high GEMINI_RPM=1`, then `finder.py judge2 run --eval-set` with `--background file --background-file judge2_background.local.md`, then `judge2 eval` with the same flags. Same prompt template and background sheet as rounds 5-6; only the thinking level differs, and it joins the prompt_version hash, so this is a NEW prompt_version (printed on the `judge2.run: ... reviewed` line). It holds the DuckDB write lock until it ends: no other `finder.py` command, and no read-only connect, will work meanwhile.
**Log.** `db/judge2_live_eval7_20260920.log`. Progress lines `judge2.run: n/90`; finished when the last line is `=== ALL DONE <date> ===`. Pace ~3.1-3.5 min per posting (about 2.5 min of reasoning + the 1/min pacing), 11/90 at 19:40, so expect the end between 23:50 and 00:20 EDT. Check: `tail -3 db/judge2_live_eval7_20260920.log` and `ps -eo pid,etime,cmd | grep "finder.py judge2"`. If it died partway, rerun the same command: reviewed rows are cached by (posting, hash, prompt_version) and are skipped.
**Why.** The user's hypothesis: minimal thinking handicaps the model. Round 6 proved the model is deterministic at temperature 0, so any difference from round 5 (catch 62% / agree 78%, 39/64 gold lines) is the effect of thinking alone.
**When it ends.** (1) Read the `judge2.evaluate` line, the CATCH/AGREE MISS lines and the line-level report in the log. (2) `finder.py judge2 eval --compare 0d6b4871f44a <round-7 prompt_version>` for the rows that changed. (3) Report to the user: numbers vs round 5 and vs the 70/85 bar, which misses moved, the cost (~5 h per 90 postings vs ~1.75 h). (4) Delete `db/judge2_r7.sh`, refresh this file, `.personal_patterns` scan, push (local commits not yet on origin: `a6bc224` thinking-aware output cap / pacing allowance / 900 s read timeout, plus this note).
**Still waiting on the user (not blocking the run).** Rulings on the remaining misses: whether an or-list years line inside a domain the candidate has not worked in can be met by a generic listed item; three gold labels that conflict with the user's own rulings (held clearance after day 1, a tool named in the title, a finance-function role); two facts the background sheet may be missing (technology-transformation years, project-management years). Open code items: two gold `meets` rows got zero `required` lines (no call); one posting unparseable in rounds 5-6 (possible output-cap truncation).
